...
class ShareRepository():
    ...
    def has_not_share(self, note_id:id, user_id:int, role:str | None = None) -> bool:
    ...
    def has_any_label_share(self, label_ids:list[int], user_id:int, role:str | None = None)-> None:
    ...
    def list_note_ids_shared_directly(self, user_id:int)->list[int]:
        return self.db.exec(
            select(NoteShare.note_id).where(NoteShare.user_id == user_id)).all()
    ...
    def list_label_ids_shared_with_user(self, user_id:int)->list[int]:
        return self.db.exec(
            select(LabelShare.label_id).where(
                LabelShare.user_id == user_id
            )).all()
