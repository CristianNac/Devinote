...
class NoteService():
    ...
    def user_can_read(self, user_id:int, note: Note)->bool:
        ...
        if self.shares.has_not_share(note_id = note.id, user_id = user_id):
            return True
    ...
    def user_can_edit(self, user_id: int, note:Note)->bool:
        ...
        if self.shares.has_not_share(note_id = note.id, user_id = user_id, role = ShareRole.EDIT):
            return True
    ...
    def _set_labels(self, owner_id:int, note_id:int, label_ids:list[int])->None:
        ...
        self.notes.replace_labels(owner_id, note_id, label_ids=valid_ids)
